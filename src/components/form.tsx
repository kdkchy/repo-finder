...
export default function Form(props: { result: (data: IGithubUser[]) => void }) {
  const [search, setSearch] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [submitState, setSubmitState] = useState<ESubmitState>(
    ESubmitState.IDLE,
  );
      ...
      <div className="flex flex-col items-center mb-4 w-full space-y-1">
        <div className="flex w-full xl:w-2/3 space-x-2">
          <div className="w-full">
            <label
              className={`input w-full flex items-center ${submitState == ESubmitState.INVALID ? 'border-red-500 border-2' : 'border-slate-200'}`}
            >
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setSubmitState(ESubmitState.IDLE);
                }}
                className="w-full"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
            ...
            <p className="invisible lg:visible text-sm italic flex justify-end text-slate-500 mt-1">
              <CornerDownLeft size={15} className="mr-1" /> Press enter to
              search
            </p>
            ...
            {submitState == ESubmitState.INVALID && (
              <p className="text-white text-sm italic -mt-8 flex ">
                <div className=" bg-red-500 flex p-1 rounded-md shadow-lg pt-3 px-2">
                  <CircleOffIcon size={15} className="mr-1" /> Oops! That didn't
                  work. Try a valid GitHub username.
                </div>
              </p>
            )}
          </div>
