...
export async function searchGitHubUsers(query: string, limit = 5): Promise<IGithubUser[]> {
    try {
...
export async function getUserRepositories(username: string): Promise<IGithubRepo[]> {
    try {
        const response = await fetch(
